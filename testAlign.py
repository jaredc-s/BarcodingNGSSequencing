    ...
    for row in range(1,len2+1):
        ...
        for col in range(1,len1+1):
            ...
            if (value == valN):
                dir[row][col]= ['N']
                gapOpen=False
            if (value == valW) and not gapOpen:
                dir[row][col]= ['W']
                gapOpen=True
            elif (value == valW) and gapOpen:
                dir[row][col]= ['W']
                gapOpen=True
    return [F, dir]
############################################################
...
def main(uniqueSeqFile,wtSeqFile,freqInput,res,freq_out):
	...
	mat=[0]*(len(wtSeq)+1)
	sh_in=[0]*(len(wtSeq)+1)
	sh_del=[0]*(len(wtSeq)+1)
	in_freq=[0]*(len(wtSeq)+1)
	del_freq=[0]*(len(wtSeq)+1)
	for line in f2:
	...
	c2.writerow(['length']+range(1,len(wtSeq)+1))
